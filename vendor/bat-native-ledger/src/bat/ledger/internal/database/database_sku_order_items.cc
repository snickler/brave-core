...
const char table_name_[] = "sku_order_items";
...
bool DatabaseSKUOrderItems::CreateIndexV19(ledger::DBTransaction* transaction) {
  ...
  bool success = this->InsertIndex(transaction, table_name_, "order_id");

  if (!success) {
    return false;
  }

  return this->InsertIndex(transaction, table_name_, "order_item_id");
}
