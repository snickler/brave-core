...
export function App (props: AppProps) {
  ...
  if (!rateInfo || !walletInfo || !orderInfo) {
    ...
    return (
      <DialogFrame
        ...
        onClose={props.host.onDialogClose}
      >
        Loading...
      </DialogFrame>
    )
  ...
  // TODO(zenparsing): If rewards are not enabled then the user does
  // not have a wallet and there does not appear to be a way to get
  // the exchange rates that are required for credit card payments
  // (both direct payments and "add funds")
    ...
    >
      {
        flowState === 'start' ?
          <PaymentMethodPanel
            ...
            onPayWithCreditCard={props.host.onPayWithCreditCard}
            onPayWithWallet={props.host.onPayWithWallet}
